...
public class Program {
    ...
    public static void dynamicSearch(GeneratorTourist tpGenerator, BufferedReader reader) throws IOException {
        List<Tourist> listType = tpGenerator.searchByType(readType(reader));
        if (listType.size() != 0) {
            Tourist[] tpTypeList = (Tourist[]) listType.toArray(new Tourist[0]);
            GeneratorTourist tpType = new GeneratorTourist(tpTypeList);
            List<Tourist> listTransport = tpType.searchByTransport(readTransport(reader));
            if (listTransport.size() != 0) {
                Tourist[] tpTransportList = (Tourist[]) listTransport.toArray(new Tourist[0]);
                GeneratorTourist tpTransport = new GeneratorTourist(tpTransportList);
                List<Tourist> listNutrition = tpTransport.searchByNutrition(readNutrition(reader));
                if (listNutrition.size() != 0) {
                    Tourist[] tpNumberDaysList = (Tourist[]) listNutrition.toArray(new Tourist[0]);
                    GeneratorTourist tpNumberDays = new GeneratorTourist(tpNumberDaysList);
                    List<Tourist> listNumbersDays = tpNumberDays.searchByNumberDaysInterval(readIntervalNumberDays(reader));
                }
            }
        }
    }
    ...
    public static void searchSelectMethod(GeneratorTourist tpGenerator, BufferedReader reader) throws IOException {
    ...
    public static void sortSelectMethod(GeneratorTourist tpGenerator, BufferedReader reader) throws IOException {
    ...
    public static void main(String[] args) throws IOException {
        ...
        GeneratorTourist tpGenerator = new GeneratorTourist(tpSet);
